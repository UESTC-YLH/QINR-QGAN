...
def train(classes_str, dataset_str, patches, layers, n_data_qubits, batch_size, out_folder, checkpoint, randn, patch_shape, qcritic):
    ...
    all_ssim = []
    all_cos_sim = []
    out_dir = f"{out_folder}/dig_num_{classes_str}_patch_num_{patches}_layers_num_{layers}_batch_size_num_{batch_size}_gen_K_{n_gen_step}_{dataset_str}_lr_G_{lr_G}_lr_D_{lr_D}_noise"
    if randn:
        out_dir += "_randn"
