...
parser.add_argument('--type', type=str, default='sound')#image, sound
...
parser.add_argument('--in_features', type=int, default=1) #in_features=1 sound; in_features=2 image
...
parser.add_argument('--use_noise', type=float, default=1)
...
class Trainer(object):
    def __init__(self, model, criterion, optimizer, dataloader, epochs, lr_scheduler, epoch_til_summary, use_cuda=False):
        self.model = model
    ...
    def train(self, epoch):
        ...
        for step, (x, y) in enumerate(self.dataloader):
            # 看看y是个什么， x就是-1到1的1000个从小到大的值
            # a = np.squeeze(x.detach().numpy())
            # plt.subplot(1, 3, 1)
            # plt.plot(a[:, 0])
            # plt.subplot(1, 3, 2)
            # plt.plot(a[:, 1])
            # b = np.squeeze(y.detach().numpy())
            # plt.subplot(1, 3, 3)
            # plt.plot(b)
            # plt.show()
            if self.use_cuda:
...
if __name__ == "__main__":
    ...
    if args.model == 'siren_bn':
        model = Siren_bn(in_features=args.in_features, out_features=1, hidden_features=args.hidden_features, hidden_layers=args.hidden_layers, first_omega_0=args.first_omega_0, hidden_omega_0=args.hidden_omega_0, activ='sine')

    elif args.model == 'relu':
        model = Siren_bn(in_features=args.in_features, out_features=1, hidden_features=args.hidden_features, hidden_layers=args.hidden_layers, first_omega_0=args.first_omega_0, hidden_omega_0=args.hidden_omega_0, activ='relu')

    elif args.model == 'tanh':
        model = Siren_bn(in_features=args.in_features, out_features=1, hidden_features=args.hidden_features, hidden_layers=args.hidden_layers, first_omega_0=args.first_omega_0, hidden_omega_0=args.hidden_omega_0, activ='tanh')

    elif args.model == 'siren':
        model = Siren(in_features=args.in_features, out_features=1, hidden_features=args.hidden_features, hidden_layers=args.hidden_layers, first_omega_0=args.first_omega_0, hidden_omega_0=args.hidden_omega_0)

    elif args.model == 'ffn':
        model = FFN(in_features=args.in_features, out_features=1, hidden_features=args.hidden_features, hidden_layers=args.hidden_layers)

    elif args.model == 'hybridren':
        model = Hybridren(in_features=args.in_features, out_features=1, hidden_features=args.hidden_features, hidden_layers=args.hidden_layers, spectrum_layer=args.spectrum_layer, use_noise=args.use_noise)

    elif args.model == 'relu+rff':
        model = Siren_bn(in_features=args.in_features, out_features=1, hidden_features=args.hidden_features, hidden_layers=args.hidden_layers, first_omega_0=args.first_omega_0, hidden_omega_0=args.hidden_omega_0, activ='relu', rff=True)
    ...
    if args.optimizer == 'sgd':
        optimizer = torch.optim.SGD((param for param in model.parameters() if param.requires_grad), lr=args.learning_rate)

    elif args.optimizer == 'adam':
        optimizer = torch.optim.Adam((param for param in model.parameters() if param.requires_grad), lr=args.learning_rate)
    ...
    trainer = Trainer(model, criterion, optimizer, dataloader, args.epochs, lr_scheduler, args.epoch_til_summary, use_cuda=args.use_cuda)
    trainer.run()
